...
layout(set = 0, binding = 1) buffer CellBuffer { uint cells[]; }
cell_buffer;

layout(set = 0, binding = 2) buffer PointBuffer { vec2 points[]; }
point_buffer;
...
float
dist_to_line(vec2 a, vec2 b, vec2 p)
{
    vec2 dir  = b - a;
...
float
dist_to_bezier2(vec2 p0, vec2 p1, vec2 p2, float t, vec2 p)
{
    vec2 q0 = mix(p0, p1, t);
    vec2 q1 = mix(p1, p2, t);
...
void
process_bezier2(vec2 p, uint i, inout float min_udist, inout float v)
{
...
void
process_bezier2_loop(
    vec2 p, uint begin, uint end, inout float min_udist, inout float v)
{
    for (uint i = begin; i < end; i += 2) process_bezier2(p, i, min_udist, v);
}

float
cell_signed_dist(uint point_offset, uint cell, vec2 p)
{
